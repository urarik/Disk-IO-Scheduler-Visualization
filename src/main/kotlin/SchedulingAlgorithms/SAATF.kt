...
class SAATF(requestList: List<Request>, position: Int, currentSector: Int, private val min: Int, private val max: Int) :
    Scheduling(requestList.size, position, currentSector) {

    private val queue = mutableListOf<Request>()
    private val requests = mutableListOf<Request>()
    private val lowerLimit = tracksPerSurface * 0.1
    private val upperLimit = tracksPerSurface * 0.9
    private val lowerSlope = (min-max)/lowerLimit
    private val upperSlope = (max-min)/(size-1 - upperLimit)
    ...
    private fun getAccessTime(request: Request): Double {
        return with(request) {
            ...
            if(lowerLimit > track) {
                seekTime -= seekTime * ((lowerSlope*track + max)*0.01)
            } else if (upperLimit < track) {
                seekTime -= seekTime*((upperSlope * track + min)*0.01)
            }
